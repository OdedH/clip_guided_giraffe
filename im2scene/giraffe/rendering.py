...
class Renderer(object):
    ...
    def custom_render_full_visualization(self, img_out_path,
                                  render_program=['object_rotation'], injected_tuple=None, name="test"):
        for rp in render_program:
            if rp == 'object_rotation':
                self.set_random_seed()
                self.custom_render_object_rotation(img_out_path, injected_tuple=injected_tuple,
                                                   name=name)
            # if rp == 'object_translation_horizontal':
            #     self.set_random_seed()
            #     self.render_object_translation_horizontal(img_out_path)
    ...
    def custom_render_object_rotation(self, img_out_path, batch_size=1, n_steps=32, injected_tuple=None,
        ...
        # Get Random codes and bg rotation
        # latent_codes = gen.get_latent_codes(batch_size, tmp=self.sample_tmp)
        z_shape_obj, z_app_obj, z_shape_bg, z_app_bg = injected_tuple
        ...
        for step in range(n_steps):
            ...
            out.append(out_i.cpu())
        out = torch.stack(out)
        out_folder = join(img_out_path, name)
        makedirs(out_folder, exist_ok=True)
        self.save_video_and_images(
            out, out_folder, name=name,
            is_full_rotation=is_full_rotation,
            add_reverse=(not is_full_rotation))
    ...
    def render_object_with_injected_latent_zs(self, batch_size=15, z_shape_injected=None, z_app_injected=None,
                                              bg_shape_injected=None, bg_app_injected=None):
        gen = self.generator
        n_boxes = gen.bounding_box_generator.n_boxes
        ...
        if z_shape_injected is not None:
            z_shape_obj_1 = z_shape_injected

        if z_app_injected is not None:
            z_app_obj_1 = z_app_injected

        if bg_shape_injected is not None:
            z_shape_bg_1 = bg_shape_injected

        if bg_app_injected is not None:
            z_app_bg_1 = bg_app_injected
        ...
        latent_codes = [z_shape_obj_1, z_app_obj_1, z_shape_bg_1,
                        z_app_bg_1]
        # with torch.no_grad():
        return gen(batch_size, latent_codes, camera_matrices,
                   transformations, bg_rotation, mode='val')
